...
function buildHVACTable(data) {
    ...
    data.forEach(d => {
        ...
        d.raw.forEach((v, i) => { 
            const st = d.res[i].s;
            h += `<td class="${d.res[i].c}"><span class="badge badge-in-cell badge-${st==='ok'?'success':(st==='warning'?'warning':'danger')}">${st==='ok'?'정상':'이상'}</span><div class="time-val">${formatToHMS(v)}</div></td>`; 
        });
...
function buildAirTable(data) {
    ...
    data.forEach(d => {
        ...
        d.units.forEach(u => { h += `<div class="unit-box ${u.res.c}"><strong>${u.label}</strong><div class="unit-time">${formatToHMS(u.val)}</div><span class="badge-in-cell badge badge-${u.res.s==='ok'?'success':'danger'}" style="margin-top:4px; font-size:0.65rem; padding:2px 6px;">${u.res.s==='ok'?'정상':'이상'}</span></div>`; });
        h += `</div></td></tr>`;
    });
    return h + `</tbody></table></div>`;
}
