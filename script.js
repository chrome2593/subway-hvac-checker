...
function renderAll(hvac, air) {
    ...
    const buildSum = (br) => {
        ...
        let h = `<div class="summary-card ${isOk?'ok':''}"><h3>📍 ${br.name}</h3>`;
        if (isOk) h += `<div style="color:var(--success); font-weight:700;">✅ 이상 없음</div>`;
        else {
            if (cH.length > 0) {
                h += `<div><strong>[공조기]</strong><br>`;
                cH.forEach(d => { d.res.forEach((r, i) => { if (r.s === 'critical') h += `<span class="badge badge-danger">${d.name} ${hLabels[i]} (${formatToHMS(d.raw[i])})</span> `; }); });
                h += `</div>`;
            }
            if (cA.length > 0) {
                h += `<div style="margin-top:10px;"><strong>[청정기]</strong><br>`;
    ...
    document.getElementById('full-list-area').innerHTML = `
        <div class="section-header"><h2 class="section-title">📊 승강장 공조기</h2><button class="toggle-btn" onclick="toggleSection('hvac-grid', this)">상세 시간 보기</button></div>
        <div id="hvac-grid" class="equipment-row">
            <div class="branch-section"><div class="branch-label">${b.left.name}</div>${buildHVACTable(lH)}</div>
            <div class="branch-section"><div class="branch-label">${b.right.name}</div>${buildHVACTable(rH)}</div>
        </div>
        <div class="section-header"><h2 class="section-title">🌬️ 공기청정기</h2><button class="toggle-btn" onclick="toggleSection('air-grid', this)">상세 시간 보기</button></div>
        <div id="air-grid" class="equipment-row">
            <div class="branch-section"><div class="branch-label">${b.left.name}</div>${buildAirTable(lA)}</div>
            <div class="branch-section"><div class="branch-label">${b.right.name}</div>${buildAirTable(rA)}</div>
        </div>`;
}
...
function buildHVACTable(data) {
    ...
    data.forEach(d => {
        ...
        d.raw.forEach((v, i) => { 
            const st = d.res[i].s;
            h += `<td class="${d.res[i].c}"><span class="badge badge-${st==='ok'?'success':(st==='warning'?'warning':'danger')} badge-in-cell">${st==='ok'?'정상':(st==='warning'?'이상':'심각')}</span><div class="time-val">${formatToHMS(v)}</div></td>`; 
        });
...
function buildAirTable(data) {
    ...
    data.forEach(d => {
        ...
        d.units.forEach(u => { h += `<div class="unit-box ${u.res.c}"><strong>${u.label}</strong><div class="time-val">${formatToHMS(u.val)}</div><span class="badge-in-cell badge badge-${u.res.s==='ok'?'success':(u.res.s==='warning'?'warning':'danger')}" style="margin-top:4px;">${u.res.s==='ok'?'정상':'이상'}</span></div>`; });
        h += `</div></td></tr>`;
    });
    return h + `</tbody></table></div>`;
}
...
function formatL2HVAC(d, map, rules) { const ty = map[d.name] || "default"; const tg = rules[ty] || (isCooling ? rules["type3"] : rules["type4"]); const raw = [d.ls, d.lue, d.lle, d.rs, d.rue, d.rle]; const res = [analyze(raw[0], tg.s, d.name, 'supply'), analyze(raw[1], tg.e, d.name, 'exhaust'), analyze(raw[2], tg.e, d.name, 'exhaust'), analyze(raw[3], tg.s, d.name, 'supply'), analyze(raw[4], tg.e, d.name, 'exhaust'), analyze(raw[5], tg.e, d.name, 'exhaust')]; return { name: d.name, raw, res, isAb: res.some(r => r.s !== 'ok'), isCri: res.some(r => r.s === 'critical') }; }
