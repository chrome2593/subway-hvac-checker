...
function formatL2HVAC(d) {
    ...
    const targetVal = isCooling ? (CONFIG.COOLING_TARGETS[CONFIG.STATION_MAP_COOLING[d.name]] || CONFIG.COOLING_TARGETS["type3"]) : CONFIG.NORMAL_TARGETS[ty];
    ...
    const res = raw.map((v, i) => {
        ...
        return analyze(val, targetVal, d.name, type);
    });
...
function getL2Vent(sheet) {
    ...
    CONFIG.LINE2_STATIONS.forEach(stName => {
        ...
        if (stName === "반월당") {
            ...
            data.push({ name: stName, units: [{l:"시급",v:r[0],r:rs[0]},{l:"시배",v:r[1],r:res[1]},{l:"종급",v:r[2],r:rs[2]},{l:"종배",v:r[3],r:rs[3]}], isCri: rs.some(x=>x.s==='critical') });
            return;
        }
        let fRow = -1;
...
function renderAll(hvac, air, vent) {
    ...
    const buildSummary = (br) => {
        ...
        if (cV.length > 0) {
            ...
            cV.forEach(d => { 
                const criUnits = (currentLine==='line1') ? d.units.filter(u=>u.r.s==='critical') : d.units.filter(u=>u.r.s==='critical');
                criUnits.forEach(u => { h += `<span class="badge badge-danger">${d.name} ${u.l} (${formatToHMS(u.v)})</span> `; });
            });
            h += `</div>`;
        }
        if (cA.length > 0) {
            ...
            if (fRow !== -1) {
                ...
                for (let i = 5; i <= 30; i++) {
                    [4, 9].forEach(colIdx => {
                        ...
                        if (v && v !== '0' && v !== '-') { let res = analyze(v, target, stName, 'supply', true); stObj.units.push({ label: (uN || (i-4)) + "호기", val:v, res }); if (res.s !== 'ok') stObj.isAb = true; if (res.s === 'critical') stObj.isCri = true; }
                    });
                }
