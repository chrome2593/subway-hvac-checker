...
function renderAll(hvac, air) {
    ...
    if (hvacCri.length === 0 && airCri.length === 0) {
        sumHtml = `<div class="summary-container" style="border-color:var(--success); border-left-color:var(--success); background:#f0fdf4;"><div class="summary-section-title" style="color:var(--success); border-bottom:none;">✅ 모든 장비가 정상입니다.</div></div>`;
    } else {
        if (hvacCri.length > 0) {
            sumHtml += `<div style="margin-bottom:15px;"><strong>[승강장 공조기]</strong><div style="margin-top:8px; display:flex; flex-wrap:wrap; gap:6px;">`;
            hvacCri.forEach(d => { d.res.forEach((r, i) => { if (r.s === 'critical') sumHtml += `<span class="badge badge-danger">${d.name} ${hLabels[i]} (${formatToHMS(d.raw[i])})</span>`; }); });
            sumHtml += `</div></div>`;
        }
        if (airCri.length > 0) {
            sumHtml += `<div><strong>[공기청정기]</strong><div style="margin-top:8px; display:flex; flex-wrap:wrap; gap:6px;">`;
...
function buildHVACTable(data) {
    const headers = currentLine === 'line1' ? ['역사', '시점급', '시점배', '종점급', '종점배', '판정'] : ['역사', '시급', '시상', '시하', '종급', '종상', '종하', '판정'];
...
function buildAirTable(data) {
    let h = `<div class="table-wrapper"><table><thead><tr><th style="width:80px;">역사</th><th>상세 가동 현황</th></tr></thead><tbody>`;
    data.forEach(d => {
