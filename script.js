...
// 1호선 환기실을 2호선과 동일한 units 구조로 통일
function getL1Vent(sheet) {
    ...
    for (let c = 4; c <= range.e.c; c++) {
        ...
        const rs = [analyzeVent(raw[0], false), analyzeVent(raw[1], false), analyzeVent(raw[2], true), analyzeVent(raw[3], true)];
        data.push({ name: n, units: [{l:"시급",v:raw[0],r:rs[0]},{l:"시배",v:raw[1],r:rs[1]},{l:"종급",v:raw[2],r:rs[2]},{l:"종배",v:raw[3],r:rs[3]}], isCri: rs.some(r=>r.s==='critical') });
    }
    [{n:"설화명곡", c:5}, {n:"화원", c:4}].forEach(s => {
        const raw = [getCV(sheet, 98, s.c), getCV(sheet, 99, s.c), getCV(sheet, 100, s.c), getCV(sheet, 101, s.c)];
        const rs = [analyzeVent(raw[0], false), analyzeVent(raw[1], false), analyzeVent(raw[2], true), analyzeVent(raw[3], true)];
        data.push({ name: s.n, units: [{l:"시급",v:raw[0],r:rs[0]},{l:"시배",v:raw[1],r:res[1]},{l:"종급",v:raw[2],r:res[2]},{l:"종배",v:raw[3],r:res[3]}], isCri: rs.some(r=>r.s==='critical') });
    });
    return data;
}
...
function renderAll(hvac, air, vent) {
    ...
    const buildColHtml = (br, h, v, a, side) => {
        ...
        <div class="branch-column">
            ...
            <div id="${side}-vent" class="detail-content">${buildVentTable(v)}</div>
