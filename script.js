...
function analyzeVent(val, isRight, station, unitName = "") {
    ...
    if ((station === "중앙로" && isRight) || (station === "칠성시장" && !isRight) || (station === "방촌" && !isRight) || (station === "수성알파" && isRight) || (station === "송현" && isRight) || (station === "연호" && isRight) || (station === "반월당" && currentLine === 'line2') || (station === "수성구청" && isRight && unitName.includes("배기"))) {
        target = 19;
    }
    const diff = Math.abs(h - target);
...
function getL2Vent(sheet) {
    ...
    CONFIG.LINE2_STATIONS.forEach(stName => {
        ...
        if (fRow !== -1) {
            ...
            if (units.length > 0) {
                data.push({ name: stName, raw: units.map(u=>u.v), res: units.map(u=>u.r), unitNames: units.map(u=>u.l), isCri: units.some(u=>u.r.s==='critical') });
            }
        }
    });
    return data;
...
function renderAll(hvac, air, vent) {
    ...
    const vLabels = ["시급", "시배", "종급", "종배", "기타1", "기타2"];
    ...
    const buildSummary = (br) => {
        ...
        if (isOk) return h + `<div style="color:var(--success); font-weight:700;">✅ 모든 관할 장비 정상 가동 중</div></div>`;
        if (cH.length > 0) {
        ...
        if (cV.length > 0) {
            ...
            cV.forEach(d => { d.res.forEach((r, i) => { if (r.s === 'critical') {
                const lbl = (currentLine==='line1'||!d.unitNames) ? vLabels[i] : d.unitNames[i];
    ...
    const buildColHtml = (br, h, v, a, side) => {
        ...
        <div class="branch-column">
            ...
            <div id="${side}-vent" class="detail-content">${buildTable(v, vLabels)}</div>
