...
function runIntegratedAnalysis(wb) {
    ...
    // 환기실 계절 자동 판정
    if (CONFIG.VENT_SEASON.INTER.includes(m)) ventSeason = '간절기';
...
function analyzeVent(val, isRight, station, unitName = "") {
    ...
    // 2. [예외 2] 대실, 범어 (온도 검출 방식 - 임계치 판정)
    if (station === "대실" || station === "범어") {
        const limit = (ventSeason === '하절기') ? 20 : 5;
        if (h >= limit) return { s: 'warning', c: 'bad-val' };
        return { s: 'ok', c: '' };
    }

    // 3. [예외 1 & 4] 하절기 전용 특수 기준
    if (ventSeason === '하절기') {
        if (isRight && CONFIG.VENT_EXC_17_8H.includes(station)) target = 17.8;
        if (station === "고산" && isRight) target = 6.5;
        if (station === "신매" && !isRight) target = 5.5;
        if (station === "정평" && isRight) target = 5.5;
        if (station === "이곡") target = 4.5;
    }

    // 4. [예외 3] 19시간 고정 기준 (위치 조건 포함)
    const isStart = !isRight;
    if ((station === "중앙로" && isRight) || 
        (station === "칠성시장" && isStart) || 
        (station === "방촌" && isStart) ||
        (station === "수성알파시티" && isRight) ||
        (station === "송현" && isRight) ||
        (station === "연호" && isRight) ||
        (station === "반월당" && currentLine === 'line2') ||
        (station === "수성구청" && isRight && unitName.includes("배기"))) {
        target = 19;
    }

    // 5. 최종 판정 (기준치 대비 2시간 오차 시 심각)
...
function getL2Vent(sheet) {
    ...
    CONFIG.LINE2_STATIONS.forEach(stName => {
        ...
        if (stName === "반월당") {
            ...
            const res = [analyzeVent(raw[0],false,stName), analyzeVent(raw[1],false,stName), analyzeVent(raw[2],true,stName), analyzeVent(raw[3],true,stName)];
        ...
        if (fRow !== -1) {
            ...
            if (units.length > 0) data.push({ name: stName, units, isCri: units.some(u=>u.r.s==='critical') });
        }
    });
    return data;
}
...
function renderAll(hvac, air, vent) {
    ...
    const buildSummary = (br) => {
        ...
        if (cV.length > 0) {
            ...
            cV.forEach(d => { d.units.forEach(u => { if (u.r.s === 'critical') h += `<span class="badge badge-danger">${d.name} ${u.l} (${formatToHMS(u.v)})</span> `; }); });
            h += `</div>`;
        }
        if (cA.length > 0) {
...
// 헬퍼: 환기실 가로 표 (역사별 Rowspan 적용)
function buildFlexTable(data) {
    if (data.length === 0) return "<p style='padding:20px; text-align:center; color:#94a3b8;'>데이터 없음</p>";
    let h = `<div class="table-wrapper"><table><thead><tr><th style="width:90px;">역사</th><th>장비/시간</th><th>판정</th></tr></thead><tbody>`;
    data.forEach(d => {
        d.units.forEach((u, idx) => {
            h += `<tr>`;
            if (idx === 0) h += `<td class="st-name" rowspan="${d.units.length}">${d.name}</td>`;
            h += `<td class="${u.r.c}">${u.l}: ${u.v==="N/A"?"-":formatToHMS(u.v)}</td>`;
            if (idx === 0) h += `<td rowspan="${d.units.length}"><span class="badge badge-${d.isCri?'danger':'success'}">${d.isCri?'이상':'정상'}</span></td>`;
            h += `</tr>`;
        });
    });
    return h + `</tbody></table></div>`;
...
// 공조기 추출 로직 (불변)
function getL1HVAC(sheet) { const data = []; const rules = isCooling ? CONFIG.COOLING_TARGETS : CONFIG.NORMAL_TARGETS; [4, 5].forEach(col => { let n = (col === 4) ? "설화명곡" : "화원"; data.push(getL1HVAC_Obj(sheet, n, col, 81, 82, 89, 90, rules)); }); const range = XLSX.utils.decode_range(sheet['!ref']); for (let c = 4; c <= range.e.c; c++) { let n = cleanText(getCV(sheet, 0, c)); if(!n || ["합계","명곡","화원"].includes(n)) continue; data.push(getL1HVAC_Obj(sheet, n, c, 5, 6, 13, 14, rules)); } return data; }
function getL1HVAC_Obj(sheet, n, c, ls, le, rs, re, rules) { let sk = (n === "반월당" && currentLine === 'line1') ? "반월당(1호선)" : n; const ty = CONFIG.STATION_MAP_NORMAL[sk] || "type4"; const tg = isCooling ? (CONFIG.COOLING_TARGETS[CONFIG.STATION_MAP_COOLING[n]] || rules["type3"]) : rules[ty]; const raw = [getCV(sheet, ls, c), getCV(sheet, le, c), getCV(sheet, rs, c), getCV(sheet, re, c)]; const res = [analyze(raw[0], tg, n, 'supply'), analyze(raw[1], 1, n, 'exhaust'), analyze(raw[2], tg, n, 'supply'), analyze(raw[3], 1, n, 'exhaust')]; return { name:n, raw, res, isCri: res.some(r => r.s === 'critical'), isAb: res.some(r => r.s !== 'ok') }; }
...
function formatL2HVAC(d) { const ty = CONFIG.STATION_MAP_NORMAL[d.name] || "type4"; const tg = isCooling ? (CONFIG.COOLING_TARGETS[CONFIG.STATION_MAP_COOLING[d.name]] || CONFIG.COOLING_TARGETS["type3"]) : CONFIG.NORMAL_TARGETS[ty]; const raw = [d.ls, d.lue, d.lle, d.rs, d.rue, d.rle]; const res = raw.map((v, i) => { let type = (i === 0 || i === 3) ? 'supply' : 'exhaust'; let val = v; if (CONFIG.NO_EQUIPMENT[d.name] && (i === 2 || i === 5)) val = "N/A"; return analyze(val, tg, d.name, type); }); return { name: d.name, raw: raw.map((v,i)=>(CONFIG.NO_EQUIPMENT[d.name] && (i===2||i===5))?"N/A":v), res, isCri: res.some(r=>r.s==='critical'), isAb: res.some(r=>r.s!=='ok') }; }
